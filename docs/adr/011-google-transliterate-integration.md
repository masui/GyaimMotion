...
- `google.com/transliterate` (旧): オリジナルGyaimMotionで使用。動作するが非公式・将来廃止リスク
- `inputtools.google.com/request` (新、**採用**): Google Input Toolsの公式エンドポイント。候補数指定(`num`)、エラーステータス付きレスポンス(`["SUCCESS", [...]]`)、積極的にメンテナンスされている

### 複数セグメント対応 (Issue #14)

Google Input Tools APIは`["SUCCESS",[["ますい",["増井","桝井"],...],["としゆき",["俊之","敏之"],...]]]`のように複数セグメントを返す。各セグメントの候補を直積（cartesian product）で結合する。組み合わせ爆発を防ぐためlimit=20で制限。
...
### 悪い点
- Google Input Tools APIの可用性・レート制限に依存
